...
- [x] To increase the scope of GMAIL query, we can ignore big tables and process all emails from channeli (pic), (currently only emails with subject submission of bio data are being processsed). Also we can ensure each new line is in next line.
- [x] GMAIL token expiry handling.
- [x] Test the workflow on some real devices.
...
- [ ] Google cloud console, will give errors when many users will be there. (Either make this project more robust or just setup this project on own for gmail api.)
